...
import { AlertTriangle, ArrowRight, ChevronDown, CircleDollarSign, CreditCard, ExternalLink, ImagePlus, Landmark, LayoutDashboard, LogOut, Minus, Package, Plus, Search, Settings, ShieldCheck, ShoppingBag, Sparkles, TrendingUp, Truck, Trash2 } from "lucide-react";
import "./store.css";
...
export function CheckoutPage() {
  ...
  const total = lines.reduce((sum, line) => sum + line.product.price * line.quantity, 0);
  const [paymentData,setPaymentData]=useState<any>(null);const [paymentMethod,setPaymentMethod]=useState("");
  useEffect(()=>{api<any>("/api/payment-methods").then(data=>{setPaymentData(data);setPaymentMethod(data.methods.find((m:any)=>m.id===data.activeProvider)?.id||data.methods[0]?.id||"")}).catch(()=>{})},[]);
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if(paymentMethod!=="manual"){const payment=await api<{checkoutUrl:string}>("/api/payments/initialize",{method:"POST",body:JSON.stringify({orderNumber:order.order_number,checkoutToken:(order as any).checkout_token,provider:paymentMethod})});window.location.assign(payment.checkoutUrl);return}
      clear();
    ...
    <StoreShell>
      <section className="checkout">
        <form onSubmit={submit}>
          ...
          <fieldset className="payment-methods"><legend>Payment method</legend>{paymentData?.methods?.map((method:any)=><label key={method.id} className={paymentMethod===method.id?"selected":""}><input type="radio" name="payment_method" value={method.id} checked={paymentMethod===method.id} onChange={()=>setPaymentMethod(method.id)}/>{method.online?<CreditCard/>:<Landmark/>}<span><b>{method.label}</b><small>{method.online?`${paymentData.mode==="test"?"Test mode · ":""}Secure hosted payment`:method.instructions}</small></span></label>)}</fieldset>
          ...
          <button className="store-button wide" disabled={submitting||!paymentMethod}>{submitting ? "Processing…" : paymentMethod==="manual"?"Place order":"Continue to secure payment"}</button>
          <small className="checkout-submit-note"><ShieldCheck /> Card details are entered only on the selected provider’s secure checkout.</small>
        </form>
        <aside>
          <h2>Your order</h2>
...
export function OrderSuccess(){const q=new URLSearchParams(location.search),payment=q.get("payment"),{clear}=useCart();useEffect(()=>{if(payment==="paid")clear()},[payment]);return <StoreShell><section className="success"><Package/><span>{payment==="paid"?"PAYMENT CONFIRMED":"ORDER RECEIVED"}</span><h1>{payment==="paid"?"Payment successful.":payment==="failed"?"Payment needs attention.":"Thank you for your order."}</h1><p>{payment==="paid"?"Your payment was securely verified. The DE_JOY team will prepare your order.":payment==="failed"?"We could not verify this payment. No duplicate charge will be created. Contact DE_JOY with the payment reference shown below.":<>Your reference is <b>{q.get("order")}</b>. The DE_JOY team will contact you with payment and fulfilment details.</>} {q.get("reference")&&<b>{q.get("reference")}</b>}</p><Link className="store-button" to="/store">Return to store</Link></section></StoreShell>}

type AdminProduct=Partial<Product>&{images?:Product["images"]};
export function Admin(){const [state,setState]=useState<"loading"|"setup"|"login"|"ready">("loading");const [tab,setTab]=useState("overview");const [admin,setAdmin]=useState<any>(null);useEffect(()=>{Promise.all([api<{configured:boolean}>("/api/setup/status"),api<any>("/api/admin/me").catch(()=>null)]).then(([s,me])=>{setAdmin(me);setState(me?"ready":s.configured?"login":"setup")})},[]);if(state==="loading")return <div className="admin-auth"><div className="admin-spinner"/>Preparing your workspace…</div>;if(state!=="ready")return <AdminAuth mode={state} onDone={()=>location.reload()}/>;const links=[['overview','Overview',<LayoutDashboard/>],['products','Products',<Package/>],['orders','Orders',<ShoppingBag/>],['payments','Payments',<CreditCard/>],['settings','Settings',<Settings/>],['security','Security',<ShieldCheck/>]];return <div className="admin"><aside><div><Link className="logo" to="/"><b>DE_JOY</b><span>COMMERCE</span></Link><small>Store administration</small></div><nav>{links.map(([id,label,icon])=><button key={String(id)} className={tab===id?'active':''} onClick={()=>setTab(String(id))}>{icon}{label}</button>)}</nav><div className="admin-sidebar-bottom"><a href="/store" target="_blank">View live store <ExternalLink/></a><div className="admin-user"><span>{(admin?.displayName||admin?.email||'A').slice(0,1).toUpperCase()}</span><div><b>{admin?.displayName||'Store owner'}</b><small>{admin?.email}</small></div></div><button onClick={()=>api("/api/admin/logout",{method:"POST"}).then(()=>setState("login"))}><LogOut/>Sign out</button></div></aside><main>{tab==='overview'?<AdminOverview go={setTab}/>:tab==="products"?<AdminProducts/>:tab==="orders"?<AdminOrders/>:tab==="payments"?<AdminPayments/>:tab==="settings"?<AdminSettings/>:<AdminSecurity/>}</main></div>}
...
function AdminPayments(){const [data,setData]=useState<any>(null),[notice,setNotice]=useState("");const load=()=>api<any>("/api/admin/payment-settings").then(setData);useEffect(()=>{load()},[]);if(!data)return <div className="admin-skeleton">Loading payment settings…</div>;const s=data.settings;async function save(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);try{await api("/api/admin/payment-settings",{method:"PUT",body:JSON.stringify({activeProvider:f.get("activeProvider"),mode:f.get("mode"),paystackEnabled:f.has("paystackEnabled"),flutterwaveEnabled:f.has("flutterwaveEnabled"),manualEnabled:f.has("manualEnabled"),manualLabel:f.get("manualLabel"),bankName:f.get("bankName"),accountName:f.get("accountName"),accountNumber:f.get("accountNumber"),instructions:f.get("instructions")})});setNotice("Payment settings saved.");await load()}catch(error){setNotice((error as Error).message)}}const gateway=(id:string,label:string)=><article className="gateway-row"><div className={`gateway-mark ${id}`}>{id==="paystack"?"P":"F"}</div><div><b>{label}</b><small>{data.readiness[id].configured?`${s.mode} credentials detected`:`${s.mode} credentials missing`}</small></div><span className={`gateway-status ${data.readiness[id].configured?"ready":"missing"}`}>{data.readiness[id].configured?"Ready":"Action required"}</span></article>;return <><header className="admin-head"><div><span>PAYMENT OPERATIONS</span><h1>Payments</h1><p>Configure checkout methods, gateway readiness and manual settlement.</p></div></header>{notice&&<div className="admin-notice success">{notice}<button onClick={()=>setNotice("")}>×</button></div>}<form className="payment-settings" onSubmit={save}><section className="admin-card"><div className="card-title"><div><h2>Gateway status</h2><p>Credentials are stored securely in the server environment and never displayed here.</p></div><select name="mode" defaultValue={s.mode}><option value="test">Test mode</option><option value="live">Live mode</option></select></div>{gateway("paystack","Paystack")}{gateway("flutterwave","Flutterwave")}<article className="gateway-row"><div className="gateway-mark manual"><Landmark/></div><div><b>Manual payment</b><small>{data.readiness.manual.configured?"Bank details complete":"Bank details incomplete"}</small></div><span className={`gateway-status ${data.readiness.manual.configured?"ready":"missing"}`}>{data.readiness.manual.configured?"Ready":"Action required"}</span></article></section><section className="admin-card"><h2>Checkout methods</h2><label className="switch-row"><span><b>Paystack</b><small>Cards, transfers and supported Nigerian methods</small></span><input name="paystackEnabled" type="checkbox" defaultChecked={s.paystackEnabled}/></label><label className="switch-row"><span><b>Flutterwave</b><small>African and international payment methods</small></span><input name="flutterwaveEnabled" type="checkbox" defaultChecked={s.flutterwaveEnabled}/></label><label className="switch-row"><span><b>Manual payment</b><small>Bank transfer with personal confirmation</small></span><input name="manualEnabled" type="checkbox" defaultChecked={s.manualEnabled}/></label><label>Active online gateway<select name="activeProvider" defaultValue={s.activeProvider}><option value="paystack">Paystack</option><option value="flutterwave">Flutterwave</option></select></label></section><section className="admin-card"><h2>Manual bank transfer</h2><div className="form-grid"><label>Checkout label<input name="manualLabel" defaultValue={s.manualLabel}/></label><label>Bank name<input name="bankName" defaultValue={s.bankName}/></label><label>Account name<input name="accountName" defaultValue={s.accountName}/></label><label>Account number<input name="accountNumber" inputMode="numeric" defaultValue={s.accountNumber}/></label><label className="wide">Customer instructions<textarea name="instructions" defaultValue={s.instructions}/></label></div></section><button className="store-button">Save payment settings</button></form><section className="admin-card payment-transactions"><h2>Recent transactions</h2><div className="transaction-row headings"><span>Reference</span><span>Provider</span><span>Order</span><span>Amount</span><span>Status</span></div>{data.transactions.map((t:any)=><article className="transaction-row" key={t.id}><code>{t.reference}</code><span>{t.provider}</span><span>#{t.order_number}</span><b>{money(t.amount_minor)}</b><span className={`gateway-status ${t.status==="successful"?"ready":"missing"}`}>{t.status}</span></article>)}{!data.transactions.length&&<p>No gateway transactions yet.</p>}</section></>}
