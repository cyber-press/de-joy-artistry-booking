...
app.get("/api/categories",asyncRoute(async(_req,res)=>{const r=await query(`SELECT c.*,(SELECT COUNT(*)::int FROM products p WHERE p.category=c.name AND p.status='active') product_count FROM categories c WHERE c.status='active' ORDER BY c.sort_order,c.name`);res.json({categories:r.rows});}));
...
app.get("/api/admin/categories",requireAdmin,asyncRoute(async(_req,res)=>{const r=await query(`SELECT c.*,(SELECT COUNT(*)::int FROM products p WHERE p.category=c.name AND p.status<>'archived') product_count FROM categories c ORDER BY c.sort_order,c.name`);res.json({categories:r.rows});}));
app.post("/api/admin/categories",requireAdmin,asyncRoute(async(req,res)=>{const input=z.object({name:z.string().min(2).max(80),slug:z.string().max(100).optional(),description:z.string().max(500).default(''),image_url:z.string().max(1000).default(''),status:z.enum(['active','archived']).default('active'),sort_order:z.coerce.number().int().min(0).default(0)}).parse(req.body);const r=await query(`INSERT INTO categories(id,name,slug,description,image_url,status,sort_order) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,[crypto.randomUUID(),input.name,slugify(input.slug||input.name),input.description,input.image_url,input.status,input.sort_order]);res.status(201).json(r.rows[0]);}));
app.put("/api/admin/categories/:id",requireAdmin,asyncRoute(async(req,res)=>{const input=z.object({name:z.string().min(2).max(80),slug:z.string().max(100).optional(),description:z.string().max(500).default(''),image_url:z.string().max(1000).default(''),status:z.enum(['active','archived']).default('active'),sort_order:z.coerce.number().int().min(0).default(0),previous_name:z.string().optional()}).parse(req.body);const client=await pool.connect();try{await client.query('BEGIN');const current=await client.query('SELECT name FROM categories WHERE id=$1 FOR UPDATE',[req.params.id]);if(!current.rowCount){await client.query('ROLLBACK');return res.status(404).json({error:'Category not found'});}const oldName=current.rows[0].name;const r=await client.query(`UPDATE categories SET name=$2,slug=$3,description=$4,image_url=$5,status=$6,sort_order=$7,updated_at=now() WHERE id=$1 RETURNING *`,[req.params.id,input.name,slugify(input.slug||input.name),input.description,input.image_url,input.status,input.sort_order]);if(oldName!==input.name)await client.query('UPDATE products SET category=$2,updated_at=now() WHERE category=$1',[oldName,input.name]);await client.query('COMMIT');res.json(r.rows[0]);}catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}}));
app.delete("/api/admin/categories/:id",requireAdmin,asyncRoute(async(req,res)=>{const r=await query<any>(`SELECT c.id,(SELECT COUNT(*)::int FROM products p WHERE p.category=c.name AND p.status<>'archived') product_count FROM categories c WHERE c.id=$1`,[req.params.id]);if(!r.rowCount)return res.status(404).json({error:'Category not found'});if(r.rows[0].product_count>0)return res.status(409).json({error:'Move or archive the products in this category first'});await query('DELETE FROM categories WHERE id=$1',[req.params.id]);res.status(204).end();}));
...
app.post("/api/admin/products/:id/images", requireAdmin, upload.array("images",8), asyncRoute(async (req, res) => {
  const files = req.files as Express.Multer.File[]; const existing = await query<any>("SELECT COALESCE(MAX(position),-1) position,COUNT(*)::int count FROM product_images WHERE product_id=$1", [req.params.id]);
  if(existing.rows[0].count+files.length>8){await Promise.all(files.map(file=>fs.promises.unlink(file.path).catch(()=>{})));return res.status(400).json({error:"A product can have up to eight images"});}
...
app.patch("/api/admin/images/:id",requireAdmin,asyncRoute(async(req,res)=>{const input=z.object({alt_text:z.string().max(180).optional(),position:z.coerce.number().int().min(0).optional()}).parse(req.body);const r=await query(`UPDATE product_images SET alt_text=COALESCE($2,alt_text),position=COALESCE($3,position) WHERE id=$1 RETURNING *`,[req.params.id,input.alt_text??null,input.position??null]);if(!r.rowCount)return res.status(404).json({error:'Image not found'});res.json(r.rows[0]);}));
